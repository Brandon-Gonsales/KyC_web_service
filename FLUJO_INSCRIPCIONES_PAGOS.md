...
4. [Fase 3: Inscripción](#fase-3-inscripción)
5. [Fase 4: Pagos](#fase-4-pagos)
...
| **Sistema** | Automatización | Calcular precios, actualizar estados, validar datos |
...
- `descuento_curso`: Descuento aplicable a todos (opcional)
...
#### Paso 3: Aplicar descuento del curso
```
Descuento curso = 3000 × 10% = 300 Bs
Precio con descuento curso = 3000 - 300 = 2700 Bs
```

#### Paso 4: Aplicar descuento personalizado
```
Descuento personal = 2700 × 5% = 135 Bs
...
### 4.1. Pago de MATRÍCULA

#### 4.1.1. Estudiante sube comprobante
...
```json
{
  ...
  "cantidad_pago": 500,
  "comprobante_url": "https://res.cloudinary.com/.../voucher_matricula.pdf"
}
```
...
#### 4.1.2. Admin revisa pagos pendientes
...
3. Confirma que el pago existe

---

#### 4.1.3. Admin APRUEBA el pago
...
#### 4.1.4. O si el comprobante es inválido...
...
monto_por_cuota = (2565 - 500) / 5 = 413 Bs
```

---

#### 4.2.1. Estudiante paga CUOTA 1
...
```json
{
  ...
  "concepto": "CUOTA",
  "numero_cuota": 1,
  "numero_transaccion": "TRX-XYZ789012",
  "cantidad_pago": 413,
  "comprobante_url": "https://res.cloudinary.com/.../voucher_cuota1.pdf"
}
```

---

#### 4.2.2. Admin aprueba CUOTA 1
...
```json
{
  "total_pagado": 913,      // 500 + 413
  "saldo_pendiente": 1652,  // 2565 - 913
...
#### 4.2.3. Proceso se repite para CUOTAS 2, 3, 4, 5

| Cuota | Monto | Total Pagado | Saldo Pendiente | Estado |
|-------|-------|--------------|-----------------|--------|
| Matrícula | 500 | 500 | 2065 | `activo` |
| Cuota 1 | 413 | 913 | 1652 | `activo` |
| Cuota 2 | 413 | 1326 | 1239 | `activo` |
| Cuota 3 | 413 | 1739 | 826 | `activo` |
| Cuota 4 | 413 | 2152 | 413 | `activo` |
| Cuota 5 | 413 | 2565 | 0 | `completado` ✅ |

**Cuando `saldo_pendiente` llega a 0:**
- Estado cambia automáticamente: `ACTIVO` → `COMPLETADO`
...
└─────────────────────────────────────────────────────────────┘
                            ↓
        Admin crea Course con precios
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ FASE 3: INSCRIPCIÓN                                         │
└─────────────────────────────────────────────────────────────┘
                            ↓
        Admin crea Enrollment
                            ↓
        Sistema calcula precios con descuentos
                            ↓
...
└─────────────────────────────────────────────────────────────┘
        ...
        └─────────────────────────────────────┘
                            ↓
        Student sube comprobante MATRICULA
                            ↓
    ...
    Enrollment.estado   Student ve motivo
       = ACTIVO         Puede subir otro
        ...
        └─────────────────────────────────────┘
              ↓
        Student sube comprobante CUOTA X
              ↓
        Admin aprueba
              ↓
        Actualiza saldo
...
### ¿Cómo funciona el descuento personalizado?
